...
### ⚠️ State=Switch — Workaround (no native variant)

```kotlin
// No PopChipVariant.Switch exists — manage active state externally
...
### Chip Stack — Count=Plenty (scrollable, with right overflow on initial render)
...
### Chip Stack — Both overflows active (mid-scroll state)
...
| `Is active=False` | `isActive = false` (default) | PopChipConfig.kt |
| `Is disabled=True` | `enabled = false` | PopChipConfig.kt |
| `Is disabled=False` | `enabled = true` (default) | PopChipConfig.kt |
| `L-Icon=True` | `leadingIcon = PopIconConfig(icon = Icons.*)` | PopChipConfig.kt |
| `L-Icon=False` | `leadingIcon = null` (default) | PopChipConfig.kt |
...
| `Divider=True` | `showDivider = true` (default) | PopChipStack.kt |
| `Divider=False` | `showDivider = false` | PopChipStack.kt |
...
> **Never** place individual `PopChip` composables side-by-side manually — always use `PopChipStack`.
> **Never** hardcode spacing, padding, or border values — all tokens come from `PopSpacing`, `PopStroke`, `BorderColor`, `SurfaceColor`.
> **Never** replicate the active border animation — it is built into `PopChip` and fires automatically from `isActive` + `mode`.
> **Never** use raw `Box` + `Text` for a chip — always use `PopChip(config = PopChipConfig(...))`.
> **Never** use `PopChipStackSize.*` on a standalone `PopChip` — it only applies to `PopChipStack`.
> **Always** wire `onCloseClick` when using `PopChipVariant.WithClose` — leaving it null makes the ✕ icon non-functional.
> **Always** use `showRightOverflow = true` on initial render for `Count=Plenty` stacks.
