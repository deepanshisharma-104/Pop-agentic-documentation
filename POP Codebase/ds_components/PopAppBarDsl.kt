...
fun ConfigureAppBar(
    ...
    // Keep references fresh across recompositions so the lifecycle observer
    // and LaunchedEffect never use stale parameter captures.
    val currentNavigationIcon by rememberUpdatedState(navigationIcon)
    val currentOnNavigationClick by rememberUpdatedState(onNavigationClick)
    val currentShowNavigationIcon by rememberUpdatedState(showNavigationIcon)
    val currentNavigationIconTint by rememberUpdatedState(navigationIconTint)
    ...
    fun applyConfigIfActive() {
        ...
        if (isLifecycleActive) {
            ...
            appBarManager?.updateNavigation(
                ...
                onNavigationClick = currentOnNavigationClick,
                showNavigationIcon = currentShowNavigationIcon,
                navigationIconTint = currentNavigationIconTint,
                ownerKey = ownerKey
            )
        }

        // Update icon slots if provided (also only when lifecycle is active)
        if (currentBlock != null && isLifecycleActive) {
            val builder = AppBarIconSlotsBuilder()
            currentBlock!!.invoke(builder)
...
fun ConfigureAppBarRightSlot(
    ...
    fun applyRightSlotIfActive() {
        ...
        if (isLifecycleActive) {
            appBarManager?.updateRightSlot(ownerKey = ownerKey, rightSlot = currentContent)
        }
    }
...
fun ConfigureAppBarLeftSlot(
    ...
    fun applyLeftSlotIfActive() {
        ...
        if (isLifecycleActive) {
            appBarManager?.updateLeftSlot(ownerKey = ownerKey, leftSlot = currentContent)
        }
    }
